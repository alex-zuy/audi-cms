...
const PhotoForm = React.createClass({
    ...
    checkPhotoIdAndFillForm(photoId) {
        if(!_.isNull(photoId)) {
            ...
            this.setState({previousImageURL: PUBLIC_URL + jsRoutes.controllers.Photos.showImage(photoId).url});
        }
    },
    isFileChosen() {
...
function getTimestampedImageUrl(photoId) {
    return PUBLIC_URL + jsRoutes.controllers.Photos.showImage(photoId).url + `?timestamp=${new Date().getTime()}}`;
}
